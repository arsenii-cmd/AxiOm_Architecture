...
> Актуально на **08.06.2026**, обновлено **26.06.2026** (DPI-фикс: транспорт `[tcp]` Reality → XHTTP+TLS, см. §2), **30.06.2026** (добавлен Naive-прокси — нативная интеграция в Marzneshin + поддержка в клиенте, см. §9 и [`NAIVE_PROXY.md`](NAIVE_PROXY.md)), **01.07.2026** (Naive раскатан на RU, см. §9; разобран и починен инцидент с недоступностью WS на PL/RU, см. §10). Единый источник истины по тому, **что есть сейчас**
...
**Архитектура (идентична на FR/PL/NL/RU):**
...
**Раскатка:** идентично на PL/FR/NL/RU параметризованным чеклистом (домен ноды → свой LE-серт,
переиспользуется тот, что уже держит Caddy на ноде для других целей). RU раскатан **01.07.2026**
как реакция на инцидент с DPI (см. §10) — на момент раскатки уже готовый LE-серт под `ru.DOMAIN`
нашёлся сразу, доп. выпуска не требовалось.
...
## 10. Инцидент: WS не работал на PL/RU (01.07.2026)

**Симптом:** жалоба на таймауты `[ws]` на Poland и Russia. Диагностика — read-only SSH на все
4 ноды, без изменений на этапе разбора.

**Что оказалось НЕ причиной:** Caddy тут ни при чём — `[ws]`-транспорт голый (`security: none`),
слушает напрямую xray на порту 9001, Caddy обслуживает только `[tcp]`-хосты через 443. Ноды в
панели показывали `status: healthy` (хотя в поле `message` у всех висело `"timeout"` — не
диагностический сигнал сам по себе).

**Poland:** трафик по `node_usages` упал в ноль в момент планового рестарта xray-процесса внутри
контейнера `marznode` (02:17 01.07). Обычный `docker restart marzneshin-node-marznode-1`
восстановил живой трафик мгновенно (подтверждено `accepted`-записями в access-логе сразу после
рестарта).

**Russia:** тот же рестарт **не помог** — реджекты `invalid request user id` продолжались, в том
числе с внутреннего IP туннеля (`10.8.0.2`). Проверка через синтетический VLESS-WS клиент
(поднятый `xray run` внутри контейнера) с реальным UUID из актуальной подписки показала: **сама
нода полностью исправна** — CONNECT проходит и локально, и снаружи (тест с FR). Проблема была не
в конфиге/сервере.

**Реальная причина — вероятный DPI.** Реджекты на RU шли с российских IP (подтверждено geoip, не
сканеры). Правдоподобное объяснение: RU-нода — единственная из четырёх на IP внутри российского
адресного пространства, поэтому для локальной DPI/провайдерской фильтрации она куда более лёгкая и
приоритетная мишень, чем зарубежные PL/FR/NL — блокировать/инспектировать национальный хостинг
проще и дешевле (юрисдикция, отсутствие пересечения международных стыков), чем избирательно резать
диапазоны зарубежных дата-центров. Голый `[ws]` без TLS — самая уязвимая для такой инспекции
сигнатура; `[tcp]` тоже мог не проходить из-за отсутствия `fp=chrome` на TLS ClientHello (тот же
открытый TODO из §9).

**Итог/фикс:** раскатан **Naive-прокси на RU** (см. §9) как транспорт, устойчивый к этому классу
DPI — честный TLS до реального домена, HTTP/2 CONNECT поверх него. Подтверждён end-to-end тестом
(CONNECT через `gost` с реальным паролем пользователя, снаружи, дал `200 Connection established`).
