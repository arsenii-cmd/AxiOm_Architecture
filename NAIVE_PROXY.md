...
> Дата внедрения: **30.06.2026** (PL, FR, NL), **01.07.2026** (RU — как реакция на инцидент с
> DPI-блокировкой `[ws]`/`[tcp]` на этой ноде, см. `MARZNESHIN_STATE.md` §10). Раскатано на
> **всех четырёх нодах** одинаково.
...
## 3. Серверная часть (на каждой ноде: FR, PL, NL, RU)
...
Сервис **Naive** создан через API панели одинаково на всех четырёх нодах (PL/FR/NL/RU). Хосты
...
1. Убедиться, что на ноде есть валидный LE-сертификат на её домен (Caddy на ноде уже должен
   его держать для других целей — gost переиспользует те же файлы; на практике сертификаты Caddy
   лежат по пути `/var/lib/caddy/.local/share/caddy/certificates/acme-v02.api.letsencrypt.org-directory/<домен>/<домен>.crt`/`.key`, а не `fullchain.pem`/`privkey.pem`, как можно было бы
   предположить — путь зависит от способа выпуска, сверяться по факту на ноде).
2. Установить `gost` (бинарник, версия должна совпадать с остальными нодами — проще скопировать
   готовый бинарник с рабочей ноды, чем качать заново) и `python3-xxhash` (`apt-get install`).
3. Развернуть `naive_auth.py`, `/etc/naive-auth.env` (креды панели, `NAIVE_SERVICE_IDS` —
   **идентичны на всех нодах**, не per-нода значения, можно просто скопировать файл), `/etc/gost/naive.yaml` (путь к серту — **единственное**, что меняется от ноды к ноде), юниты
   `naive-auth.service` + `gost-proxy.service`, `systemctl daemon-reload && enable --now` оба.
4. На панели (один раз, не per-нода): сервис **Naive** уже существует, привязать к нему новый
   хост ноды через `POST /api/inbounds/hosts` (не `/api/hosts` — это не тот путь несмотря на
   интуитивно ожидаемое имя), `service_ids` — те же ID, что и у остальных нод (в текущей
   конфигурации `[3, 6]`), remark — `<Страна> ({USERNAME}) [Naive]`.
5. Проверить логи `naive-auth`/`gost-proxy` — `naive-auth` должен залогировать `[refresh] N valid
   users` (N > 0 — значит успешно достучался до API панели и нашёл юзеров нужного сервиса).
6. **Тестовый юзер должен реально состоять в сервисе Naive** (`service_ids` содержит `3` или `6`
   в `users_services`) — иначе он не увидит Naive-хост в подписке вообще, это не баг раскатки.
   End-to-end проверка без установки клиента: поднять внутри `marznode`-контейнера сырой
   `CONNECT`-запрос к `https://<домен-ноды>:8888` с `Proxy-Authorization: Basic base64(axiom:<password
   из sing-box-подписки тестового юзера>)` — ответ `HTTP/1.1 200 Connection established` от
   `Proxy-Agent: gost/3.0` подтверждает всю цепочку разом (TLS, auther, проброс).

**Готча (наблюдалась при раскатке на RU):** длинные inline-команды через `ssh host "..."` с
несколькими уровнями вложенных кавычек (Python-код внутри `docker exec` внутри `ssh`) ломаются на
экранировании почти всегда — писать Python-скрипт в отдельный файл и заливать его (`scp` +
`docker cp`), а не собирать код строкой в аргументе `ssh`.

**RU раскатан 01.07.2026** — см. `MARZNESHIN_STATE.md` §10 (реакция на инцидент с DPI).
