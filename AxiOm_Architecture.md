...
- **(30.06.2026) Протокол Naive** добавлен в парсер и UI рядом с VLESS/Hysteria2 (токен `[naive]`/`[http]`, лейбл «Naive», транспорт «HTTP/2 TLS», строка «Транспорт» скрыта как у Hysteria2 — один транспорт). Виден только при sing-box-формате подписки от панели. Подробности — `MARZNESHIN_STATE.md` §9 и `NAIVE_PROXY.md`.
...
*Документ отражает состояние системы по состоянию на июнь 2026 (обновлён 01.07.2026).*

---

## 17. История изменений

### 01.07.2026 — Инцидент WS на PL/RU + раскатка Naive на RU

- **Симптом:** таймауты на транспорте `[ws]` на Poland и Russia. Caddy оказался ни при чём —
  `[ws]` идёт напрямую в xray на порту 9001 без TLS, Caddy обслуживает только `[tcp]`/443.
- **Poland:** трафик обнулился в момент планового рестарта xray-процесса внутри `marznode`;
  обычный `docker restart` контейнера нода вернул живой трафик мгновенно.
- **Russia:** рестарт не помог. Синтетический VLESS-WS клиент с реальным UUID из актуальной
  подписки подтвердил — нода и конфиг полностью исправны и локально, и при тесте снаружи (с FR).
  Реджекты `invalid request user id` шли с реальных российских IP (не сканеры, проверено geoip).
  **Вероятная причина — DPI**: RU-нода единственная из четырёх хостится на IP внутри российского
  адресного пространства, что делает её приоритетной и более дешёвой целью для локальной
  фильтрации/блокировки по сравнению с зарубежными PL/FR/NL. Голый `[ws]` без TLS — самая уязвимая
  для такой инспекции сигнатура.
- **Фикс:** раскатан **Naive-прокси на RU** (см. запись 30.06.2026 ниже, `NAIVE_PROXY.md`,
  `MARZNESHIN_STATE.md` §10) — устойчивый к этому классу DPI транспорт. Подтверждён end-to-end
  тестом: `CONNECT` через `gost` с реальным паролем пользователя, снаружи → `200 Connection
  established`. Naive теперь раскатан одинаково на всех четырёх нодах (FR/PL/NL/RU).

### 30.06.2026 — Naive-прокси (gost) — нативная интеграция в Marzneshin + поддержка в клиенте

Второй, независимый от xhttp (см. запись 26.06.2026) запасной транспорт — HTTP/2 CONNECT поверх
TLS, портирован в sing-box из Caddy `forwardproxy`. **В xray-core такого протокола нет** — это
архитектурное ограничение движка, не недоработка конфигурации.

- **Сервер (одинаково на FR/PL/NL):** отдельный процесс **gost v3.2.6** (TLS-листенер `:8888`,
  серт ноды) вместо Caddy `forwardproxy` — у того известный баг TCP-hijack для CONNECT (трафик
  не туннелировался после `200 OK`). Аутентификация — auther-плагин gost → HTTP-callback на
  `naive_auth.py` (опрашивает API панели каждые 30с, пароль = `xxh128(subscription_key)`, тот же
  алгоритм, которым сама Marzneshin деривирует секреты — `AUTH_GENERATION_ALGORITHM=XXH128`).
- **Нативная интеграция в панель без миграций схемы:** три точечных патча
  (`app/models/proxy.py`, `app/utils/share.py`, `v2share/singbox.py`) поверх существующего
  механизма «ручных хостов», применяются volume-mount в `docker-compose.yml` (переживают
  `docker pull`/пересоздание контейнера). Перед началом — полный бэкап `docker-compose.yml` и
  панели (риск для боевой системы). Создан сервис **Naive** через API одинаково на PL/NL/FR;
  хосты привязаны через `service_ids`. Формат remark — `<Страна> ({USERNAME}) [Naive]`.
- **Видимость зависит от ФОРМАТА подписки, не от тумблера клиента:** у Naive нет URI-схемы
  (`naive://` не существует), выразим только sing-box JSON. Marzneshin переключает формат через
  `subscription_settings.rules` (regex по UA, общий для всех хостов сразу). Для `xray`/`links`-
  формата Naive автоматически не появляется (генератор ссылок не знает протокола `http`,
  тихо пропускает хост) — без какого-либо патча для этого случая.
- **Клиент AxiOm v2:** добавлена обработка протокола в `server_option.dart` (токен, ранг
  сортировки, лейблы) и `server_selector_card.dart` (скрытие строки «Транспорт», как у
  Hysteria2). Полные технические детали и чеклист раскатки на новую ноду —
  [`NAIVE_PROXY.md`](NAIVE_PROXY.md); сводка состояния — `MARZNESHIN_STATE.md` §9.
- **RU раскатан 01.07.2026** — см. запись выше и `MARZNESHIN_STATE.md` §10.
- ⏭️ **Не реализовано:** fingerprint-стратегия для сторонних xray-клиентов (Happ и т.п.) —
  идея отдать им `[tcp]`-хосты с нормальным `fp=chrome` через отдельное UA-правило панели,
  не трогая то, что видит AxiOm-клиент. План — в `NAIVE_PROXY.md` §8.

### 26.06.2026 — DPI-фикс: транспорт `[tcp]` переведён Reality → XHTTP+TLS (без пересборки клиента)

- **Проблема:** ТСПУ/DPI 3-го поколения в РФ режет **VLESS+Reality** (поведенческий анализ) — слот `[tcp]` во всех тарифах перестал работать.
- **Решение (бесшовное, без пересборки приложения):** на всех 4 нодах слот `[tcp]` переведён с Reality на **VLESS + XHTTP + настоящий TLS** (домен `fr.DOMAIN`, Let's Encrypt-серт). DPI видит легитимный HTTPS к реальному домену. Новые xhttp-инбаунды на FR/PL/NL/RU (порт `2087`, `network:xhttp`, `security:tls`, `path:/xhvless`). Хосты `[tcp]` перенаправлены на них, старые reality-хосты отключены. RU `[tcp]` (xhttp) добавлен в «Максимальный».
- **Ключевой урок (удалённые ноды ≠ локальная):** на удалённых нодах (`grpclib`) ручная правка `xray_config.json` поднимает xray, но **не регистрирует инбаунд для пуша юзеров** → панель не впрыскивает пользователей. Инбаунд надо задавать **через панель** (`GET`→правка→`PUT /api/nodes/{id}/xray/config`), тогда конфиг пушится И инбаунд регистрируется. На локальной ноде FR (`grpcio`) правка файла работает полностью.
- **Готчи:** `fp=chrome` (uTLS) в vless-ссылке **ломает xhttp-downstream** → fingerprint оставлять пустым; reality несовместима с xhttp. Клиент обязан включить тумблер **«Use xray-core when possible»** (Настройки→Общие), иначе sing-box-путь не туннелирует xhttp — UX-трение для существующих сборок (для прода нужна рассылка/инструкция). Правило подписки: `^HiddifyNextX → base64-links` (чтобы vless-ссылка несла `type=xhttp&security=tls&sni&path`).
- **Автоматическое продление серта:** systemd-timer `marznode-cert-sync.timer` (daily) на FR раздаёт обновлённый Let's Encrypt-серт `fr.DOMAIN` с Caddy на все ноды по SSH (только при изменении) + рестартит marznode. Без него серт ротировался бы ~раз в 60 дней и xhttp-инбаунды бы отвалились.
- **Версия ядра не при чём:** прошлый диагноз «version-lock xray 24↔25» оказался ложным — XHTTP+TLS работает на текущем ядре `v4.1.0`, блокером была именно reality-связка.
...
**Симптом:** клиент оплатил Maximum (400₽) 10.06 05:26, подписка не выдалась автоматически
(выдана вручную: `web_jh2hr7`, claim-ссылка `?start=claim_…`).

**Корень (не тот, что подозревался):** дело было НЕ в рестартах бота — журнал показал, что бот
работал непрерывно ~14 часов после оплаты, а webhook ЮKassa ретраит до 24 ч. Реальная причина —
**HTTP-уведомление вообще не было прописано в ЛК ЮKassa** (висел незакрытый ручной шаг с 31.05).
Веб-выдача держалась только на опросе статуса с лендинга → если покупатель оплачивал (СБП с
телефона) и не возвращался на страницу, платёж зависал в `pending` навсегда.

**Сделано:**
- **Webhook настроен** в ЛК (Интеграция → HTTP-уведомления): `payment.succeeded`,
  `payment.canceled`, `refund.succeeded` → `https://pay.DOMAIN/payment/webhook`. Цепочка
  `pay.DOMAIN → Caddy → бот:8080` проверена (HTTP 200).
- **Страховочный поллинг** (`bot.py`): `web_payment_poll_loop` раз в 10 мин зовёт
  `web_payment_reconcile()` — берёт `pending` веб-оплаты старше 15 мин
  (`db.get_stale_pending_web_payments`), сверяет статус в Платёжке и довыдаёт оплаченные
  (`issue_web_subscription`, идемпотентность через существующий CAS) либо помечает `canceled`.
  Теперь даже при недоставленном webhook'е деньги не виснут дольше ~10 мин. Проверено живьём:
  3 застрявших `pending` (отменённые в ЮKassa) поллинг закрыл в `canceled`.
- В `payment_webhook` добавлен лог успешной веб-выдачи (была диагностическая слепая зона).
- Сверка всех оплаченных платежей ЮKassa за 30 дней против выдачи в БД — **0 проблемных**.
- Бэкап на RU: `bak-poll-20260611-105321-{bot,db}.py`.
...
- **Webhook регистрируется вручную в ЛК** (`/v3/webhooks` секретным ключом недоступен).
  ✅ **Настроен 11.06.2026** (ЛК → Интеграция → HTTP-уведомления): `payment.succeeded`,
  `payment.canceled`, `refund.succeeded` → `https://pay.DOMAIN/payment/webhook`. До этого
  выдача держалась только на опросе статуса с лендинга (см. запись 11.06.2026 в §17).
- **Статус (01.06.2026): БОЕВОЙ режим** — магазин `SHOP_ID` (live), карта и СБП доступны,
  платежи реальные. Подробности и чеклист — `PAYMENT_INTEGRATION.md`.
